...
function showArtists(response){
	...
	$.each(artistArray.artists.items,function (index, artist){
		...
		var html = ` 
				<li>
					<h2>${artist.name}</h2>
					<img src="${artist.images[0].url}" style="width:100px" id="${artist.id}" class="get-albums">
				</li><br>
				`;
	...
	$.each(albumArray.items,function (index, album){
		var html = ` 
				<span style="padding:10px;display:inline-block">
					<h2>${album.name}</h2>
					<img src="${album.images[0].url}" style="width:100px" id="${album.id}" class="get-tracks" data-toggle="modal" data-target="#myModal">
